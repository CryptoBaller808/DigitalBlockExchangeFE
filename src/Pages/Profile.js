...
const Profile = () => {
  ...
  const get_profiles_getCollectedItemsByUserIdAndCategoryId = async ()  => {
    ...
    const res = await axios.get(`${process.env.REACT_APP_API_URL}/profiles/getCollectedItemsByUserIdAndCategoryId/?user_id=${user.id}&cat_id=${activecategory?.id}`)
  ...
  const get_profiles_getCreatedItemsByUserIdAndCategoryId = async ()  => {
    setsearchitemwithuseridorbycategoryloading(true)
    const res = await axios.get(`${process.env.REACT_APP_API_URL}/profiles/getCreatedItemsByUserIdAndCategoryId?user_id=${user.id}&cat_id=${activecategory?.id}`)
  ...
  const post_profiles_getItemsOnSaleByUserIdAndCategoryId = async ()  => {
    setsearchitemwithuseridorbycategoryloading(true)
    const res = await axios.get(`${process.env.REACT_APP_API_URL}/profiles/getItemsOnSaleByUserIdAndCategoryId?user_id=${user.id}&cat_id=${activecategory?.id}`)
  ...
  const getfavouriteByUserId = async ()  => {
    const res = await axios.get(`${process.env.REACT_APP_API_URL}/profiles/getFavoriteItemsByUserId?id=${user.id}&page_number=${1}&page_size=${100}`)
      ...
      <div className="wrapWidth wrap flex flex-col">
        <div className="user-info flex flex-col">
          ...
          <div className="user-img-sec flex rel">
            <img 
            src={`${user?.profile_image ? user?.profile_image : 'unkdownload.jpeg'}`} 
            className="user-img" />
            ...
            <div className="user-token flex aic">
              ...
              <div className="lbl">r3rhW ... w96W</div>
        ...
        <div className="profile-tabs flex aic">
          ...
          <div className={`tab-item flex`}>
            <p
              className={`lbl ${activeTab === "Collections" ? "active" : ""}`}
              onClick={(e) => setActiveTab("Collections")}
            >
              Created Collecions
            </p>
          </div>
          <div className={`tab-item flex`}>
            <p
              className={`lbl ${activeTab === "Activity" ? "active" : ""}`}
              onClick={(e) => setActiveTab("Activity")}
            >
              Activity
            </p>
          </div>
        ...
        <div className="search-box flex">
          ...
          type="text" className="txt cleanbtn" placeholder="Search" />
        </div>
        {activeTab === "Activity" ? (
              ...
              <div className="tbl flex flex-col">
                ...
                {activities.map((item, index) => (
                  <div className="row flex aic">
                    ...
                    <div className="row-item flex">{item?.qty}{console.log(item?.buyer_details)}</div>
                    <div className="row-item flex">{item?.buyer_details?.firstname} {item?.buyer_details?.lastname}</div>
                    <div className="row-item flex">{item?.seller_details?.firstname} {item?.seller_details?.lastname}</div>
            ...
            <div className="profile-page-nft">
              {
                ...
                ?
                  ...
                  :
                    ...
                    "No items found"
                    :
                    searchitemwithuseridorcategoryitems.map((item, index) => (
                      <Card item={item}/>
                    ))
                :
                !loading ?
