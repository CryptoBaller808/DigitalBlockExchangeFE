...
const Swap = () => {
  ...
  //   // setIsTransaction(false)
  //   console.log('time out after 10 secs')
  // }, 1000 * 60)
  ...
  return (
    <div className="swap-page flex">
      {isTransaction && <SwapTransModal swapfrom={swapFrom.value} swapTo={finalExchangeRate} swapFromCurrency={swapFrom.currency} swapToCurrency={swapTo.currency} />}
        ...
        <ReactModal.Body>
          <div className="d-flex flex-column justify-content-center align-items-center">
            <div className="d-flex mb-2 justify-content-center">
              <p className="me-2 text-center">{(Number.isInteger(Number(swapFrom.value))) ? swapFrom.value : parseFloat(swapFrom.value).toFixed(5)}</p>
            ...
            <div className="mb-3 align-self-center">
              <p className="mb-0" style={{ color: "#54626F" }}>Output is estimated, final result will depend on XRP Ledger</p>
            </div>

            <div className="text-center p-3 d-flex justify-content-between align-items-center w-100 rounded" style={{ backgroundColor: '#f1f1f1' }}>
              <p className="mb-0">Rate</p>
              <p className="mb-0">1 {swapFrom.currency} = {(exchangeRate?.exchangeRateForOne).toFixed(5)} {swapTo.currency}</p>
            </div>
          </div>
      ...
      <div className="wrap wrapWidth flex aic flex-col">
        ...
        {success.length !== 0 && <Alert variant="success" onClose={() => setSuccess('')} dismissible>
          <p>{success}</p>
        </Alert>}
            ...
            <div className="card-left flex">
              <div className="card flex flex-col">
                ...
                <div className="field flex">
                  <input type="text" className="txt cleanbtn" placeholder="Swap amount" value={swapFrom.value}
                    onChange={(e) => setSwapFrom({ ...swapFrom, value: e.target.value })} pattern="\d*" maxLength="5" />
                </div>
              </div>
            </div>
