...
const Explore = () => {
  ...
  console.log('totalitems',totalitems,numbs.length)
  const getAllitemsOnSale = async ()  => {
    try {
      const res = await axios.get(`${process.env.REACT_APP_API_URL}/sale/getAllitemsOnSale?page_number=${page_number}&page_size=${page_size}`)
